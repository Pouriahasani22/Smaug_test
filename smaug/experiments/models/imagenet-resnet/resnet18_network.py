...
def conv_block(
  ...
  conv3_tensor = sg.Tensor(
      data_layout=sg.NHWC, tensor_data=generate_random_data(
          (filters1, 1, 1, input_tensor_chans)))
...
def create_resnet18():
  with sg.Graph(name="resnet18_smv", backend="SMV") as graph:
    ...
    bn0_beta_tensor = sg.Tensor(
        data_layout=sg.NC, tensor_data=generate_random_data((1, 64)))
    fc_tensor = sg.Tensor(
        data_layout=sg.NC, tensor_data=generate_random_data((10, 7 * 7 * 512)))
...
if __name__ != "main":
  ...
  resnet18.print_summary()
  resnet18.write_graph()
